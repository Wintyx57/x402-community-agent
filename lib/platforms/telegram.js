...
export async function sendPreview(contents) {
  ...
  let message = `📋 Community Agent — New Post\n\n`;
  ...
  message += `Reply /approve to post or /reject to discard`;
...
// Post to public channel — returns normalized { success, message }
export async function postToChannel(text, imageUrl) {
  ...
  if (!channelId) {
    ...
    return { success: false, message: 'TELEGRAM_CHANNEL_ID not configured' };
  }
  ...
  if (imageUrl) {
    ...
    return { success: !!data.ok, message: data.ok ? 'Posted' : (data.description || 'Failed') };
  }
...
// Send copy-paste ready posts to admin (for platforms without API: Twitter, Reddit, HN)
export async function sendCopyPaste(contents) {
  if (!config.platforms.telegram.enabled) return;

  const labels = {
    twitter: '🐦 TWITTER — Copie et poste:',
    reddit: '🟠 REDDIT — Copie et poste:',
    hn: '🟧 HACKER NEWS — Copie et poste:',
  };
  ...
  for (const [platform, content] of Object.entries(contents)) {
    ...
    const data = await res.json();
    if (data.ok) log(`Copy-paste sent for ${platform}`);
    else log(`Copy-paste failed for ${platform}: ${data.description || 'unknown'}`);
  }
}

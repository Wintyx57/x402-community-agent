// Content generation pipeline — uses Gemini (primary) or x402 APIs (fallback)
import { callApi, callFreeApi } from './x402-client.js';
import { isGeminiConfigured, generateWithGemini, translateWithGemini, analyzeSentimentWithGemini } from './gemini.js';
...
// Generate text — Gemini (primary) → x402 /api/summarize (fallback) → local fallback
export async function generateText(prompt, maxLength = 500, localFallback = null) {
  ...
  if (isGeminiConfigured()) {
    try {
      log(`Generating text via Gemini (max ${maxLength} chars)...`);
      const text = await generateWithGemini(prompt, { maxTokens: Math.ceil(maxLength / 3), temperature: 0.8 });
      if (text && text.length > 20) return text.slice(0, maxLength);
    } catch (err) {
      log(`Gemini failed: ${err.message} — trying x402 fallback`);
    }
  }

  // Fallback to x402 API
  try {
    log(`Generating text via x402 /api/summarize (max ${maxLength} chars)...`);
    ...
    log(`x402 API call failed: ${err.message} — using local fallback`);
  }
...
// Translate text — Gemini (primary) → x402 /api/translate (fallback)
export async function translateText(text, targetLang = 'fr') {
  if (isGeminiConfigured()) {
    try {
      log(`Translating to ${targetLang} via Gemini...`);
      return await translateWithGemini(text, targetLang);
    } catch (err) { log(`Gemini translate failed: ${err.message} — trying x402`); }
  }
  if (!process.env.AGENT_PRIVATE_KEY) { log('No wallet — skipping translate'); return text; }
  try {
    log(`Translating to ${targetLang} via x402...`);
...
// Analyze sentiment — Gemini (primary) → x402 /api/sentiment (fallback)
